...
const TrendPlot = (props) => {
    ...
    useEffect(() => {
        const {api, fetchTrendSuccess, fetchTrendError, trendReducer, metricReducer, fetchMetricSuccess, fetchMetricError} = props;
    ...
    const onForecastDirection = (direction) => {
        ...
        return (
            <p className="stat-value"><span className={`stat-value-arrow __${dir.toLocaleLowerCase()}`}/></p>
        );
    };

    const {
        isLoading, trendReducer, ztLearnerReducer, metricReducer
    } = props;
        ...
        <Fragment>
            ...
            <Plot
                ...
                layout={{
                    ...
                    title: {
                        text: '<b><span style="margin-right: 2rem;">' + hours + ' Hour Target Price:' + '<span style="margin: 0 2rem; color: #fff "> BTC/USD: ' + metricReducer?.data?.predicted_price?.toFixed(2) + '</span></span></b>',
                        x: 0.05,
                        y: 0.98,
                        font: {
                            size: 16,
