...
const MarketCorrelation = (props) => {
    ...
    const {isLoading, data: {month_month_corr}} = props;
    ...
    return (
        <Fragment>
            <InfoModalGraph show={show} onPress={() => setShow(!show)}
                            text={'Return correlation explores the return\n' +
            ...
            />
            <div style={{position: 'absolute', bottom: '1rem', right: '1rem', color: '#9E9FA1'}}>Correlation</div>
        </Fragment>
    )
}
...
export default WithApiService()(connect(mapStateToProps, mapDispatchToProps)(MarketCorrelation));
