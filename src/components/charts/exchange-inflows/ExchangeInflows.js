...
} from "Store/charts/exchangeInOutFlow/actions";
...
const ExchangeInflows = (props) => {
  ...
  const onChangeFilter = (startDate) => {
    const {api, fetchExchangeInOutFlowSuccess, fetchExchangeInOutFlowError} =
      props;
    fetchData(
      api.getExchangeInOutFlow,
      fetchExchangeInOutFlowSuccess,
    ...
    <Fragment>
      ...
      <Plot
        data={[
          {
            name: "Outflow",
            y: data.data.exchange_inflow.slice(
              data.data.exchange_inflow.length - 10
            ),
          ...
          {
            name: "Inflow",
            y: data.data.exchange_outflow.slice(
              data.data.exchange_outflow.length - 10
            ),
            x: data.data.date.slice(data.data.date.length - 10),
            type: "bar",
            marker: {
...
export default WithApiService()(
  connect(mapStateToProps, mapDispatchToProps)(ExchangeInflows)
);
