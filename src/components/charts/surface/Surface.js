...
const graphData = {
    ...
    useEffect(() => {
        const {api, fetchMetricsCallsSurfaceSuccess, fetchMetricsCallsSurfaceError, data} = props;
        if (isEmpty(data)) {
            api.getMetricCallsSurface()
                .then(probData => fetchMetricsCallsSurfaceSuccess(probData))
                .catch(error => fetchMetricsCallsSurfaceError(error));
        }
    },[])

        const {isLoading, type, data: {xcalls, ycalls, zcalls, xputs, yputs, zputs, xDiff, yDiff, zDiff}} = props;
        ...
        return (
            <Fragment>
                ...
                onClick={() => setScroll(true)}
                    ...
                    layout={viewSettingsLayout(`${type} Options Volatility Surface`, true, {
                        l: 50,
                        r: 0,
                        t: 0,
                        b: 20
                    }, '', '', 0, '', '', '', '', '', '', '',
                        'Strike', 'Time to Maturity', 'Implied Volatility').layout}
...
export default WithApiService()(connect(mapStateToProps, mapDispatchToProps)(Surface));
