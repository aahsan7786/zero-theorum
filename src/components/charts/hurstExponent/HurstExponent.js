...
} from "../../../store/charts/hurstExponent/actions";
...
const HurstExponent = (props) => {
  ...
  useEffect(() => {
    const {api, fetchHurstExponentSuccess, fetchHurstExponentError, data} =
      props;

    if (isEmpty(data)) {
      fetchData(
        api.getHurstExponent,
        fetchHurstExponentSuccess,
        fetchHurstExponentError,
  ...
  return (
    <Fragment>
      <InfoModalGraph
        ...
        text={
          "Hurst Exponent values from 0 to 0.5\n" +
...
export default WithApiService()(
  connect(mapStateToProps, mapDispatchToProps)(HurstExponent)
);
