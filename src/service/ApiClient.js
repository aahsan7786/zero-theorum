...
export default class ApiClient {
  ...
  getResidualHistogram = async (startFrom) => {
    const data = await this.getSources(
      `/residual-histogram?range=${startFrom}`
    );
  ...
  getBTCRolling = async (startFrom, finishedTo) => {
    const data = await this.getSources(
      `/btc-rolling-sharpe?start_date=${startFrom}`
    );
  ...
  getCostOfProduction = async (startDate) => {
    const data = await this.getSources(
      `/btc-cost-of-production?start_date=${startDate}`
    );
  ...
  getScatteredPlots = async () => {
    const data = await this.getSources(
      `/raw-values-corr?start_date=2021-01-01`
    );
  ...
  getRollingCorrelations = async (startFrom) => {
    const data = await this.getSources(
      `/rolling-correlation?start_date=2021-01-01`
    );
  ...
  getRollingEnthropy = async (startFrom) => {
    const data = await this.getSources(
      `/btc-rolling-enthropy?start_date=${startFrom}`
    );
  ...
  getRollingNetFlow = async (startFrom) => {
    const data = await this.getSources(
      `/exchanges-in-out-rolling?start_date=${startFrom}`
    );
  ...
  getHurstExponent = async (startFrom) => {
    const data = await this.getSources(
      `/hurst-exponent?start_date=${startFrom}`
    );
  ...
  getDiscord = async () => {
    return await fetch(
      "https://discord.com/api/guilds/878488514507997225/widget.json",
      {
        method: "GET",
      }
    ).then((r) => r.json());
  ...
  login = async (username, password) => {
    return fetch(
      "https://satoshiscore.net/api/authuser?username=test&password=FORgot123",
      {
        method: "GET",
      }
    );
