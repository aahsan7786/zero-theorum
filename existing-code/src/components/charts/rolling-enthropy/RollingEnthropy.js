...
import {fetchRollingEnthropySuccess, fetchRollingEnthropyError} from 'Store/charts/rollingEnthropy/actions';
...
import information from "Images/icons/information.svg";
    ...
    useEffect(() => {
        const { api, fetchRollingEnthropySuccess, fetchRollingEnthropyError, data } = props;
        if(isEmpty(data)) {
            fetchData(api.getRollingEnthropy, fetchRollingEnthropySuccess, fetchRollingEnthropyError, transformDateFormat(lastWeek()));
        }
    }, [])

    const onChangeFilter = (startDate) => {
        const { api, fetchRollingEnthropySuccess, fetchRollingEnthropyError } = props;
        fetchData(api.getRollingEnthropy, fetchRollingEnthropySuccess, fetchRollingEnthropyError, startDate);
    }

        const {isLoading, data: {date, value}} = props;
        ...
        return (
            <Fragment>
                <InfoModalGraph show={show} onPress={() => setShow(!show)}
                                text={'Entropy is a measure of market\n' +
                ...
                <Plot
                    ...
                    layout={viewSettingsLayout('Rolling Enthropy', false, {
                        l: 40,
                        r: 30,
                        t: 95,
                        b: 30
                    }, '', '', 0.5, 'Date', 'Entropy', 'stack', 'percent', true, true).layout}
...
export default WithApiService()(connect(mapStateToProps, mapDispatchToProps)(RollingEnthropy));
