...
import {fetchBTCRollingSuccess, fetchBTCRollingError} from 'Store/charts/btcRolling/actions';
...
const BTCRolling = (props) => {
    ...
    useEffect(() => {
        ...
        if (isEmpty(data)) {
            fetchData(api.getBTCRolling, fetchBTCRollingSuccess, fetchBTCRollingError, transformDateFormat(lastWeek()));
        }
    }, [])
    ...
    const
        onChangeFilter = (startDate) => {
            ...
            fetchData(api.getBTCRolling, fetchBTCRollingSuccess, fetchBTCRollingError, startDate);
        }
    ...
    return (
        <Fragment>
            <InfoModalGraph show={show} onPress={() => setShow(!show)}
                            text={'Sharpe Ratio is the ratio of average\n' +
            ...
            <Plot
                ...
                layout={viewSettingsLayout('BTC Sharpe Ratio', true, {
                    l: 30,
                    r: 30,
                    t: 95,
                    b: 30
                }, '%y/%d/%m', '', 0, 'Date', 'Sharpe Ratio').layout}
...
export default WithApiService()(connect(mapStateToProps, mapDispatchToProps)(BTCRolling));
