...
const ImpliedVolatility = (props) => {
    ...
    useEffect(() => {
        const {api, fetchImpliedVolatilitySuccess, fetchImpliedVolatilityError, data} = props;
        if (isEmpty(data)) {
            api.getImpliedVolatility()
                .then(probData => fetchImpliedVolatilitySuccess(probData))
                .catch(error => fetchImpliedVolatilityError(error));
        }
    }, [])

    const {isLoading, data: {calls, puts, delta}} = props;
    ...
    return (
        <Fragment>
            ...
            <Plot
                ...
                layout={viewSettingsLayout(props.title, true, {
                    l: 40,
                    r: 30,
                    t: 95,
                    b: 30
                }, '', '', 0.5, 'Date', 'Volatility', 'stack', 'percent', true, true).layout}
...
export default WithApiService()(connect(mapStateToProps, mapDispatchToProps)(ImpliedVolatility));
